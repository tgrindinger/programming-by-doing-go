...
func paint(da *gtk.DrawingArea, cr *cairo.Context) {
	gp1 := NewGraphPaper(cr, 10, 10, 300, 300)
	gp1.drawBounds()
	gp1.drawLinePoints(1, 0)
	gp2 := NewGraphPaper(cr, 320, 10, 300, 300)
	gp2.drawBounds()
	gp2.drawLinePoints(-3, 4)
	gp3 := NewGraphPaper(cr, 630, 10, 300, 300)
	gp3.drawBounds()
	gp3.drawLinePoints(0.5, -3)
	gp4 := NewGraphPaper(cr, 10, 320, 300, 300)
	gp4.drawBounds()
	gp4.drawLinePoints(-2, -2)
	gp5 := NewGraphPaper(cr, 320, 320, 300, 300)
	gp5.drawBounds()
	gp5.drawLinePoints(2, -1)
	gp5.drawLinePoints(-0.5, 5)
	gp6 := NewGraphPaper(cr, 630, 320, 300, 300)
	gp6.drawBounds()
	gp6.drawLinePoints(0, 4)
}

func main() {
	gtk.Init(nil)
	win := createWindow("GraphPaper", 1100, 950)
