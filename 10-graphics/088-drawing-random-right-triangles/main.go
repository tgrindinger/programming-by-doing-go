...
func paint(da *gtk.DrawingArea, cr *cairo.Context) {
	w := float64(da.GetAllocatedWidth()) - 50
	h := float64(da.GetAllocatedHeight()) - 50
	for i := 0; i < 500; i++ {
		cr.SetSourceRGB(rand.Float64(), rand.Float64(), rand.Float64())
		x := rand.Float64() * w
		y := rand.Float64() * h
		tri := NewPolygon(cr)
		tri.addPoint(x, y)
...
func main() {
	gtk.Init(nil)
	win := createWindow("Random Triangles", 1024, 768)
