...
func drawSmilingFace(cr *cairo.Context, x, y float64) {
	cr.SetSourceRGB(1, 1, 0)
	rad := 100.0
	fillOval(cr, x - rad, y - rad, rad * 2, rad * 2)
	cr.SetSourceRGB(1, 0, 0)
	drawArc(cr, x - rad + 50, y - rad + 50, 100, 100, 45, 135)
	cr.SetSourceRGB(0, 0, 1)
	fillOval(cr, x - rad + 50, y - rad + 50, 40, 40)
	fillOval(cr, x - rad + 100, y - rad + 50, 40, 40)
}

func paint(da *gtk.DrawingArea, cr *cairo.Context) {
	drawSmilingFace(cr, 100, 100)
	drawSmilingFace(cr, 400, 350)
	drawSmilingFace(cr, 800, 150)
	drawSmilingFace(cr, 100, 550)
	drawSmilingFace(cr, 650, 600)
}

func main() {
	gtk.Init(nil)
	win := createWindow("Smiling Face Function", 1024, 768)
