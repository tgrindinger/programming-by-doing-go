...
func drawPng(cr *cairo.Context, filename string, x, y float64) {
	image, err := cairo.NewSurfaceFromPNG(filename)
	if err != nil {
		log.Fatalf("Unable to open file '%s': %s", filename, err)
	}
	cr.SetSourceSurface(image, x, y)
	fillRect(cr, x, y, float64(image.GetWidth()), float64(image.GetHeight()))
}
...
func paint(da *gtk.DrawingArea, cr *cairo.Context) {
	...
	drawPng(cr, "sierpinski.png", 100, 100)
	cr.SetSourceRGB(1, 0, 0)
	cr.SelectFontFace("Times New Roman", cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
	cr.SetFontSize(48)
	drawString(cr, "Sierpinski Gasket", 340, 750)
	cr.SelectFontFace("Century Gothic", cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
	cr.SetFontSize(16)
	drawString(cr, "It's very nifty", 450, 800)
}

func main() {
	gtk.Init(nil)
	win := createWindow("Motivational Poster", 1024, 850)
