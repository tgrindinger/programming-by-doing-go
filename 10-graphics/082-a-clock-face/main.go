...
func paint(da *gtk.DrawingArea, cr *cairo.Context) {
	...
	y := 300.0
	rad := 100.0
	drawOval(cr, x - rad, y - rad, rad * 2, rad * 2)
	drawLine(cr, x, y, x + rad * 0.6, y - rad * 0.6)
	drawLine(cr, x, y, x + rad * 0.4, y + rad * 0.4)
	drawString(cr, "12", x - 5, y - rad * 0.8)
	drawString(cr, "6", x - 5, y + rad * 0.8)
	drawString(cr, "3", x + rad * 0.8, y - 5)
	drawString(cr, "9", x - rad * 0.8, y - 5)
}
