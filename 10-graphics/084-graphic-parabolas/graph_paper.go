...
func (g *GraphPaper) drawPoint(px, py float64, visible bool) bool {
...
func (g *GraphPaper) drawLine(m, b float64) {
	g.cr.SetSourceRGB(0, 0, 1)
	visible := false
	for px := -10.0; px <= 10; px += 0.01 {
		py := m * px + b;
		visible = g.drawPoint(px, py, visible)
	}
	g.cr.Stroke()
	g.cr.ClosePath()
}

func (g *GraphPaper) drawParabola(h, k, a float64) {
	g.cr.SetSourceRGB(0, 0, 1)
	visible := false
	for px := -10.0; px <= 10; px += 0.01 {
		py := a * math.Pow(px - h, 2) + k
		visible = g.drawPoint(px, py, visible)
	}
	g.cr.Stroke()
	g.cr.ClosePath()
