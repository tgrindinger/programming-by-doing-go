...
func daEvent(da *gtk.DrawingArea, event *gdk.Event) bool {
	...
	if eventClick.Type() == gdk.EVENT_BUTTON_PRESS || eventClick.Type() == gdk.EVENT_MOTION_NOTIFY {
		lastX, lastY = eventClick.MotionVal()
		message = fmt.Sprintf("You are at (%.0f,%.0f)", lastX, lastY)
		da.QueueDraw()
		return false
	}
	return true
...
var message string = "Click someplace and drag!"
var lastX, lastY float64

func main() {
	gtk.Init(nil)
	printEventTypes()
	win := createWindow("Drag Location", 1024, 768)
