...
type IGraphics interface {
	...
	setFont(name string, slant FontSlant, weight FontWeight, size float64)
...
func (g *Graphics) setColor(color Colors) {
	...
	switch color {
		...
		case COLOR_GRAY:    c = Color{0.5, 0.5, 0.5}
...
func (g *Graphics) setColorRGB(color *Color) {
	g.cr.SetSourceRGB(color.R, color.G, color.B)
}

func (g *Graphics) setFont(name string, slant FontSlant, weight FontWeight, size float64) {
	cairoSlant := cairo.FONT_SLANT_NORMAL
	cairoWeight := cairo.FONT_WEIGHT_NORMAL
	switch slant {
	case SLANT_ITALIC: cairoSlant = cairo.FONT_SLANT_ITALIC
	case SLANT_OBLIQUE: cairoSlant = cairo.FONT_SLANT_OBLIQUE
	}
	switch weight {
	case WEIGHT_BOLD: cairoWeight = cairo.FONT_WEIGHT_BOLD
	}
	g.cr.SelectFontFace(name, cairoSlant, cairoWeight)
	g.cr.SetFontSize(size)
}

func (g *Graphics) setLineWidth(width float64) {
	g.cr.SetLineWidth(width)
}
